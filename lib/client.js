...
`;
		...
		const CAT_LEGEND = [
			["mcp", "MCP 服务"], ["skill", "Skill 工具"], ["hook", "事件钩子"], ["workflow", "Workflow"],
			["model", "模型调度&推理"], ["ui", "UI 界面"], ["system", "系统功能拓展"], ["security", "离线&安全合规"],
		];
		...
		function apply(ctx) {
			...
			function Panel() {
					...
					fetch(API + "/deps/fix", { method: "POST", headers: { "content-type": "application/json" }, body: "{}" })
						...
						.then((j) => {
							if (j.ok) {
								...
								const lines = j.value.actions.map((a) => (a.ok ? "✔ " : "✖ ") + (a.plugin === "(profile)" ? "依赖补齐" : a.plugin) + ": " + a.detail);
								setDepsMsg({ err: false, text: "修复完成（重启网关后完全生效）：\n" + lines.join("\n") });
							} else {
								setDepsMsg({ err: true, text: j.error || "修复失败" });
							}
						})
						.catch((e) => setDepsMsg({ err: true, text: String(e) }))
						.finally(() => setDepsBusy(false));
				...
				function depsSection() {
					const head = [
						h("button", { className: "po_btn po_btnGhost", onClick: scanDeps, disabled: depsBusy }, deps ? "重新扫描" : "扫描依赖"),
					];
					const nodes = [
						h("p", { className: "po_sub" }, "检查每个插件的依赖/peer 是否齐全、链接是否完好、是否存在跨插件版本冲突；可自动修复损坏的链接与缺失的依赖（解决插件启动报错）。"),
					...
					if (deps !== null) {
						...
						nodes.push(h("div", { className: "po_depsSummary" },
							h("span", { className: "po_dsItem", "data-sev": "ok" }, "✔ 正常 " + s.ok),
							h("span", { className: "po_dsItem", "data-sev": "warn" }, "⚠ 警告 " + s.warn),
							h("span", { className: "po_dsItem", "data-sev": "error" }, "✖ 问题 " + s.error),
							s.fixable > 0
								? h("button", { className: "po_btn", onClick: fixDeps, disabled: depsBusy }, "一键修复 (" + s.fixable + ")")
								: h("span", { className: "po_tag", "data-on": true }, "无待修复项")));
						if (deps.conflicts.length > 0) {
							nodes.push(h("div", { className: "po_depBlock" },
								h("div", { className: "po_depHead" },
									h("span", { className: "po_h" }, "跨插件版本冲突"),
									h("span", { className: "po_tag", "data-on": false }, deps.conflicts.length + " 处")),
								deps.conflicts.map((c) => h("p", { className: "po_depNote", key: c.name + "|" + c.a.plugin + "|" + c.b.plugin },
									"依赖 " + c.name + "：" + c.a.plugin + " 要求 " + c.a.spec + "，但 " + c.b.plugin + " 要求 " + c.b.spec + "（主版本区间无交集，可能无法同时加载）"))));
						}
						for (const p of deps.plugins) {
							...
							const depRows = p.deps.length === 0
								? [h("tr", { key: "none" }, h("td", { colSpan: 6 }, "（无第三方依赖）"))]
								: p.deps.map((d) => {
									const sev = d.optional ? (d.status === "ok" ? "ok" : "warn") : d.status === "ok" ? "ok" : d.status === "missing" || d.status === "broken" ? "error" : "warn";
									const label = d.status === "ok" ? "正常" : d.status === "missing" ? "缺失" : d.status === "broken" ? "损坏" : d.status === "mismatch" ? "版本不符" : d.status;
									return h("tr", { key: d.name },
										h("td", { className: "po_depName" }, d.name),
										h("td", null, d.kind + (d.optional ? " (可选)" : "")),
							...
							nodes.push(h("div", { className: "po_depBlock", key: p.name },
								h("div", { className: "po_depHead" },
									...
									h("span", { className: "po_spacer2" }),
									pErr > 0
										? h("span", { className: "po_tag", "data-sev": "error" }, "问题 " + pErr)
										: pWarn > 0
											? h("span", { className: "po_tag", "data-on": false }, "警告 " + pWarn)
											: h("span", { className: "po_tag", "data-on": true }, "正常")),
								h("table", { className: "po_depTable" },
									h("thead", null, h("tr", null,
										h("th", null, "依赖"), h("th", null, "类型"), h("th", null, "要求"), h("th", null, "实际"), h("th", null, "状态"), h("th", null, "说明"))),
									h("tbody", null, depRows))));
						}
					} else {
						nodes.push(h("p", { className: "po_hint" }, depsBusy ? "正在扫描依赖…" : "点击「扫描依赖」检查插件的依赖状态。"));
					}
					return h("section", { className: "po_sec" },
						sectionHeader("依赖检查", deps ? "已扫描" : "", head),
						...nodes);
				}
				...
				return h("div", { className: "po_root" },
					...
					h("section", { className: "po_sec" },
						...
						h("div", { className: "po_legend" },
							h("span", { className: "po_legendTitle" }, "分类："),
							CAT_LEGEND.map(([id, label]) => {
