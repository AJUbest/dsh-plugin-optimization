...
async function scanPluginDeps(profile, item) {
  ...
  for (const [name, e] of merged) {
    ...
    if (specs.some((s) => s.startsWith("link:"))) {
      ...
      // 优先插件目录内局部安装的依赖（link 插件的依赖装在插件目录 node_modules），
      // 其次 profile node_modules（普通安装的依赖）
      const local = await resolvePluginLocal(item.dir, name);
      const r = await resolveInstalledDep(profile, name);
      if (local.found !== null) {
        if (satisfies(local.found, spec)) { status = "ok"; found = local.found; note = "插件目录内安装"; }
        else if (r.found !== null && satisfies(r.found, spec)) { status = "ok"; found = r.found; note = "已安装于 profile"; }
        else { status = "mismatch"; found = local.found; note = "插件目录内 " + local.found + "，不满足 " + spec; }
      } else if (r.found === null) {
        status = r.broken ? "broken" : "missing";
        note = r.broken ? "依赖目录符号链接悬空（安装不完整）" : "未安装（安装不完整，可一键修复）";
      } else if (!satisfies(r.found, spec)) {
...
export { findLegacyCustom, migrateOne, parseVersion, versionGt, repoRawUrl, classifyText, classifyWeighted, classifyDir, PLUGIN_CATEGORIES, parseSemver, parseRange, satisfies, allowedMajors, scanPluginDeps, scanAllDeps, collectScanItems, resolveInstalledDep, resolveLinkTarget, resolveDshBuiltin, resolveHostProvided, resolvePluginLocal };
