    ...
    for (const name of await readdir(profilesRoot)) {
      ...
      try {
        ...
        if (deps[PLUGIN_NAME] !== undefined || bundles.includes(PLUGIN_NAME)) {
          // 打分：包含 dsh-web-app 的完整 web profile 优先
          const score = (bundles.includes("dsh-web-app") ? 2 : 0) + (bundles.includes("dsh-base") ? 1 : 0);
          if (score > bestScore) { bestScore = score; dir = candidate; }
        }
      } catch { /* 跳过无法解析的 profile */ }
    }
  } catch { /* profiles 目录不存在 */ }
  if (!dir) dir = dirname(dirname(PKG_DIR)); // 兜底：按安装位置推导
  profileCache = { dir, name: basename(dir), nodeModules: join(dir, "node_modules") };
  return profileCache;
}
...
export function apply(ctx) {
  ...
  registerRoute(API_BASE + "/state", async (req, res) => {
    ...
    try {
      ...
      const describe = async (dir, spec) => {
        ...
        // 解析真实路径（pnpm 符号链接），让目录与分区判断反映文件实际位置
        const realDir = await realpath(dir).catch(() => dir);
        return {
          name: p.name,
          version: p.version,
          ...
          disabled: entryId ? isPluginDisabled(patchText, entryId) : false,
  ...
  registerRoute(API_BASE + "/import", async (req, res) => {
    ...
    try {
      ...
      const out = await dshCli(["plugin", "--profile", (await getProfile()).name, "add", folderPath]);
  ...
  registerRoute(API_BASE + "/register", async (req, res) => {
    ...
    try {
      ...
      const out = await dshCli(["plugin", "--profile", (await getProfile()).name, "add", dir]);
  ...
  // toggle → 开启/关闭自定义插件（写 profile 的 cordis.patch.yml）
  registerRoute(API_BASE + "/toggle", async (req, res) => {
    ...
    try {
      ...
      const patchText = await readFile(join(profile.dir, "cordis.patch.yml"), "utf8").catch(() => "[]\n");
      const nowDisabled = isPluginDisabled(patchText, entryId);
      await setPluginDisabled(entryId, !nowDisabled);
      send(res, 200, { ok: true, value: { note: (nowDisabled ? "已开启" : "已关闭") + "（重启网关后生效）: " + name } });
    } catch (e) {
      send(res, 500, { ok: false, error: String((e && e.message) || e) });
    }
  });
