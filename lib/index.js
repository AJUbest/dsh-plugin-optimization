...
async function runPluginDepsInstall(pluginDir) {
  const { stdout, stderr } = await execFileAsync("pnpm", ["install", "--prod", "--config.auto-install-peers=false", "--prefer-offline"], {
    cwd: pluginDir,
    windowsHide: true,
    timeout: 300000,
...
export { findLegacyCustom, migrateOne, parseVersion, versionGt, repoRawUrl, classifyText, classifyWeighted, classifyDir, PLUGIN_CATEGORIES, parseSemver, parseRange, satisfies, allowedMajors, scanPluginDeps, scanAllDeps, collectScanItems, resolveInstalledDep, resolveLinkTarget, resolveDshBuiltin, resolveHostProvided, resolvePluginLocal, resolveCustomTop };
