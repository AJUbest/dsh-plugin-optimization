...
const PLUGIN_CATEGORIES = [
  ...
  { id: "skill", label: "Skill 工具", keywords: ["skill", "技能", "工具", "tool", "shell", "filesystem", "文件系统", "搜索", "search", "函数调用", "本地函数", "命令行"] },
  ...
  { id: "system", label: "系统功能拓展", keywords: ["python", "虚拟环境", "venv", "进程管理", "插件管理", "插件治理", "打包", "分发", "缓存", "cache", "底层", "扩展", "extension", "文件系统", "filesystem", "插件市场"] },
...
/** 对一段文本做分类匹配：命中 ≥1 个关键词的类别都返回（支持跨类）。 */
function classifyText(text) {
  ...
  for (const cat of PLUGIN_CATEGORIES) {
    ...
    for (const kw of cat.keywords) {
      // 纯 ASCII 短词用词边界正则（避免 "ui" 误命中 "build" 等）；含中文的长词直接 includes
      if (/^[\x00-\x7F]+$/.test(kw) && kw.length <= 3) {
    ...
    if (count > 0) hits.push({ id: cat.id, label: cat.label });
  }
  return hits;
}

/** 读取插件目录的 package.json + README 摘要（中英文都读，兼顾双语插件），返回分类标签。 */
async function classifyDir(dir) {
  const hay = [];
  try {
    const raw = await readFile(join(dir, "package.json"), "utf8");
    const pkg = JSON.parse(raw);
    hay.push(String(pkg.name || ""));
    hay.push(String(pkg.description || ""));
    if (Array.isArray(pkg.keywords)) hay.push(pkg.keywords.join(" "));
    if (pkg.repository && pkg.repository.url) hay.push(String(pkg.repository.url));
    let readmeBudget = 3000;
    for (const fn of ["README.md", "README.zh.md", "readme.md", "README.MD"]) {
      try {
        const t = await readFile(join(dir, fn), "utf8");
        hay.push(t.slice(0, readmeBudget));
        readmeBudget = Math.max(0, readmeBudget - t.length);
        if (readmeBudget <= 0) break;
      } catch { /* 尝试下一个文件名 */ }
    }
  } catch { /* 目录不完整则跳过 */ }
  return classifyText(hay.join("\n"));
}
...
export { findLegacyCustom, migrateOne, parseVersion, versionGt, repoRawUrl, classifyText, classifyDir, PLUGIN_CATEGORIES };
